...
team_scoring = winning_team_scoring %>% 
  ...
  mutate(diff_pct = score_diff / score) %>%
  mutate(diff_pct = if_else(win == 'lose', -1*(1+diff_pct), 1+diff_pct)) %>%
...
by_season = team_scoring %>% 
  ...
  summarize(mean_score = mean(score), sd_score = sd(score), mean_diff_pct = mean(diff_pct)) %>%
  mutate(sd_score = replace_na(sd_score, 1)) %>%
  group_by(team_id) %>%
  summarize(mean_score = mean(mean_score), sd_score = mean(sd_score), mean_diff_pct = mean(mean_diff_pct)) %>%
  mutate(sd_score = replace_na(sd_score, 1))
