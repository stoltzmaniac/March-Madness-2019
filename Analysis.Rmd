...
output %>% select(team_name1, team_name2, team1_win, predicted_difference) %>% write_csv('output.csv')
```

```{r}
ggplot(output, aes(x=seed_diff, y=predicted_difference)) + geom_point() + geom_smooth(method='lm')
```

```{r}
p = ggplot(output, aes(x=factor(seed_diff), y=predicted_difference)) + geom_violin()
p
```
